...
) : IProjectGenerator
{
    ...
    )
    {
        ...
        var processExecutionContext = await processStarterFactory.Factory()
                                                                 .StartAsync(new ProcessExecutionContext(generationContext.Path, "dotnet", string.Join(" ", arguments), new Dictionary<string, string>(), true, true)
                                                                           , cancellationToken);
        if (!processExecutionContext.HasStarted)
            throw new ProcessNotStartedException(await processExecutionContext.StdErrStream
