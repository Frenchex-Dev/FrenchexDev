...
) : IMetaSolutionDefinitionGenerator
{
    ...
    {
        var solutionGenerationResult = await solutionGenerator.GenerateAsync(metaSolutionDefinition.SolutionDefinition
        ...
        await Parallel.ForEachAsync(metaSolutionDefinition.TemplatesDefinitions, new ParallelOptions
                                                                                 {
                                                                                   ...
                                                                                   , CancellationToken
                                                                                    ...
                                                                                    {
                                                                                        var templateGenerationResult
                                                                                            = await templateGenerator
                                                                                                  .GenerateAsync(definition
                                                                                                               , new
        ...
        await Parallel.ForEachAsync(graphOfBuildGenerationOrder, new ParallelOptions
                                                                 {
                                                                     MaxDegreeOfParallelism = 100
                                                                   , CancellationToken      = cancellationToken
                                                                    ...
                                                                    ) =>
                                                                    {
                                                                        ...
                                                                        {
                                                                            var projectGenerationResult
                                                                                = await projectGenerator
                                                                                      .GenerateAsync(definition
                                                                                                   , new Project.Generator.Lib.Domain.Abstractions.GenerationContext
                                                                                                     {
                                                                                                         Path
                                                                                                             = generationContext
                                                                                                                 .Path
    ...
    private Task<IList<IList<IProjectDefinition>>> BuildGraphOfGenerationOrder(
        IList<IProjectDefinition> projectsDefinitions
    )
    {
        return Task.FromResult<IList<IList<IProjectDefinition>>>(new List<IList<IProjectDefinition>>
                                                                 {
                                                                     projectsDefinitions
                                                                 });
    }
