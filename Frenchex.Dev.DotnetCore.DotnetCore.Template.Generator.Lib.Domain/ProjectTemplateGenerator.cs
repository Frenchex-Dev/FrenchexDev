...
public class ProjectTemplateGenerator : IProjectTemplateGenerator
{
    ...
    {
        await TryAddFileGenerationAsync(
                                        Path.Join(generationContext.Path, $"{projectTemplateDefinition.Name}.csproj")
                                      , projectTemplateDefinition.Name
                                      , _ => Task.FromResult(
                                                             new GeneratedFile
                                                             {
        ...
        await TryAddFileGenerationAsync(
                                        Path.Join(generationContext.Path, ".template.config", "template.json")
                                      , "template.json"
                                      , _ => Task.FromResult(
                                                             new GeneratedFile
                                                             {
        ...
        await TryAddFileGenerationAsync(
                                        Path.Join(generationContext.Path, ".template.config", "dotnetcli.host.json")
                                      , "dotnetcli.host.json"
                                      , _ =>
                                        {
